...
def send_email(text):
    ...
    if 'nghĩa' in recipient: # 'nghĩa' ở đây là keywords để máy tiếp tục gửi email cho bạn. Bạn có thể thay cái keywords này
        ...
        mail.ehlo()
        mail.starttls()
        mail.login(email,pas) # Ở khúc này, 'xyz' là địa chỉ email của bạn, 'abc' là mật khẩu của email đó
        mail.sendmail(email, pas
                      , content.encode('utf-8')) # 'xyz' vẫn như cũ như 'abc' là địa chỉ email của người mà bạn muốn họ nhận thư nha 
        mail.close()
...
def change_wallpaper():
    ...
    urllib2.urlretrieve(photo, r"C:\Users\letungsy123\Downloads\a.png")
    image=os.path.join( r"C:\Users\letungsy123\Downloads\a.png")
